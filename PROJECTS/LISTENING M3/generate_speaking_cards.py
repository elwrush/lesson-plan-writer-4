#!/usr/bin/env python3
"""Generate morality speaking cards (It's Academic — Opium/empire lesson).
...
SET A (structured, PAIRED)  - 10 topics, each a matched pair (copy 1 / copy 2).
   ...
   Theme:      morality — 10 moral situations to discuss.

SET B (bare)                - 5 single cards (not paired) — further moral
   situations, no language support. Appearance: number + prompt + hint.
"""
...
STRUCTURED = [
    {
        "num": 1,
        "title": "Is it ever OK to break a rule to do the right thing?",
        "context": "Some rules seem unfair or harmful. A person might break one to help someone in need. Is that ever justified?",
        "opener": "Let's talk about the rules we follow.",
    },
    {
        "num": 2,
        "title": "Should you tell a lie to protect a friend?",
        "context": "Your friend did something wrong. The teacher asks you directly. Do you protect them, or tell the truth?",
        "opener": "Shall we talk about honesty?",
    },
    {
        "num": 3,
        "title": "Is it fair to judge a person by what they own?",
        "context": "Some people are treated differently because they have money. Others are looked down on for having little.",
        "opener": "Let's start with how we judge people.",
    },
    {
        "num": 4,
        "title": "Should rich people be forced to help the poor?",
        "context": "Some say the wealthy have a duty to give back. Others say it should be a free choice, not a law.",
        "opener": "Should we talk about money and duty?",
    },
    {
        "num": 5,
        "title": "Is it wrong to buy a bargain that came from unfair work?",
        "context": "Cheap clothes, phones and food are often made by people working very hard for very little money.",
        "opener": "Let's discuss the things we buy.",
    },
    {
        "num": 6,
        "title": "Is watching someone being bullied the same as joining in?",
        "context": "A group is picking on one student. You do nothing and just watch. Are you partly responsible?",
        "opener": "What do you think about standing by?",
    },
    {
        "num": 7,
        "title": "Should you keep a promise even if it now seems wrong?",
        "context": "You promised to keep a secret. Later you find out the secret could hurt someone. What do you do?",
        "opener": "Let's talk about promises.",
    },
    {
        "num": 8,
        "title": "Is a small lie ever better than a painful truth?",
        "context": "A friend asks if they look good. They have a big test tomorrow and are nervous. Is kindness more important than honesty?",
        "opener": "Shall we talk about kindness and truth?",
    },
    {
        "num": 9,
        "title": "Should you report a friend who cheated?",
        "context": "Your friend cheated on a test and got a top mark. You know the truth. Do you tell, or stay quiet?",
        "opener": "Let's start with fairness.",
    },
    {
        "num": 10,
        "title": "Is it fair that one country makes most of the money?",
        "context": "Some powerful countries got rich by taking resources from poorer ones. Is that a moral problem?",
        "opener": "Let's discuss who should have the most.",
    },
]

# ---------------------------------------------------------------------------
# SET B - 5 bare cards (not paired) — further moral situations, no language help
# ---------------------------------------------------------------------------
BARE = [
    {"num": 1, "prompt": "Would you refuse to buy something made by child workers, even if it is much cheaper?"},
    {"num": 2, "prompt": "Is it more important to be honest or to be kind? Give an example."},
    {"num": 3, "prompt": "Should a country that was once colonised be paid for what was taken from it?"},
    {"num": 4, "prompt": "Do you have a duty to speak up when you see something unfair? Why?"},
    {"num": 5, "prompt": "If you had the power to fix one injustice in the world, what would you choose and why?"},
]
BARE_HINT = "What do you think?"
...
def main():
    template = TEMPLATE
    jobs = [
        ("speaking-morality-setA-structured.pdf", build_structured_pages(STRUCTURED)),
        ("speaking-morality-setB-bare.pdf", build_bare_pages(BARE)),
    ]
    for fname, pages in jobs:
        ...
        # No Ghostscript (decision 44): the cue-card PDF is generated directly
        # by Playwright with no transparent images, so no Acrobat flatten/reorder
        # risk. Never reintroduce -qgs for these.
