#!/usr/bin/env python3
"""Renders TWO separate Reciprocal Teaching booklets (B1 and B2) using the
BOOK-REPORT-PRODUCTION report.html layout (adapted) + a New Yorker-style
pen-and-ink inset.

Each booklet:
  - carries a filled CEFR badge before the title (B1 / B2)
  - uses "Task xx:" nomenclature with strongly-boxed, ultra-visible headers
  - shows the roles, the leveled chunk text, a model discussion transcript,
    discussion questions and the discussion framework table.
...
def build_blocks(level, chunk, before, after):
    """Assemble the task-block list for one booklet (B1 or B2)."""
    return [
        {"type": "task", "num": "1", "title": "How one round works", "body": timeline_html()},

        {"type": "inset", "src": ILLO.resolve().as_uri(),
        ...
        {"type": "task", "num": "3", "title": "Read and Learn",
         "body": f"<div class='chunk-head'><span class='title'>{CHUNK_HEADING}</span></div>{chunk}"},

        {"type": "task", "num": "4", "title": "Model discussion",
         "body": f"<div class='sub-head'>&#9632; Before reading</div>{transcript(before)}"
                 + READ_MARK
                 + f"<div class='sub-head'>&#9632; After reading</div>{transcript(after)}"},

        {"type": "task", "num": "5", "title": "Discuss with your group",
...
def render_booklet(level, chunk, before, after, out_path):
    blocks = build_blocks(level, chunk, before, after)
...
def main():
    ...
    print("Rendering Reciprocal Teaching booklets...")
    render_booklet("B1", B1_CHUNK, B1_BEFORE, B1_AFTER,
                   "reading-reciprocal-teaching-B1.pdf")
    render_booklet("B2", B2_CHUNK, B2_BEFORE, B2_AFTER,
                   "reading-reciprocal-teaching-B2.pdf")
    print("Done.")
