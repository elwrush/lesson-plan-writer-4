"""Post-process the Plural -s pronunciation-noticing deck.

Idempotent: safe to re-run after every render (render.py wipes injections).
Adds the 5-minute timer to the Catch the Finals slide (game-f) and registers the
timer plugin without duplication.
"""

from pathlib import Path

path = Path("PROJECTS/PRON S_Z_IZ/slides/index.html")
