...
def gloss_word_errors(level: dict) -> list[str]:
    """Gloss words NOT present as a standalone word in the level's body.

    A missing gloss word is silently dropped by sanitise; before the sanitise
    fix it also shifted every later superscript, producing the off-by-one
    gloss-numbering bug (body "disabilities³" vs footer "3 restorative"). The
    render path hard-fails on this so a deck can never ship with broken
    glossing; --check reports it so it is caught before rendering.
    """
    import re

    body = " ".join(flatten_body(level["chunks"]))
    return [
        g["word"]
        for g in (level.get("glosses") or [])
        if not re.search(rf"(?<![A-Za-z]){re.escape(g['word'])}(?![A-Za-z])", body)
    ]
...
def main() -> None:
    ...
    if args.check:
        ...
        for lvl in levels:
            ...
            miss = gloss_word_errors(lvl)
            gkind = "gloss OK" if not miss else f"MISSING GLOSS: {miss}"
            print(f"  {lvl['cefr']}: body={words}  target={target}  cap={cap}  [{kind}]  [{gkind}]")
        return
    ...
    for lvl in levels:
        ...
        missing_glosses = gloss_word_errors(level)
        if missing_glosses:
            raise SystemExit(
                f"{level['cefr']} gloss words not found in body: {missing_glosses} — "
                "fix reading.json (add the word or remove the gloss) before rendering"
            )
